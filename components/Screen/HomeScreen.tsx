...
import { getBets } from "../../api/betService";
...
export default function HomeScreen() {
    ...
    const getSelectedBets = () => {
        switch (selected) {
          case 0: return { data: allBets, loading: loadingAll, error: errorAll };
          case 1: return { data: inProgressBets, loading: loadingProgress, error: errorProgress };
          case 2: return { data: lastWeekBets, loading: loadingWeek, error: errorWeek };
          case 3: return { data: lastMonthBets, loading: loadingMonth, error: errorMonth };
    ...
    return (
        <View style={styles.container}>
            ...
            <Text style={{color: colors.light_gray, fontFamily: 'Montserrat-Regular', paddingTop: 10, paddingBottom: 10}}>Total: <Text style={{color: colors.black, fontFamily: 'Montserrat-SemiBold'}}>{bets.length} bets</Text></Text>
            <FlatList
            data={bets}
            ...
            renderItem={({ item: bet }) => (
                ...
                {/* BACK (Edit + Delete) */}
                    <View style={styles.container_swipe}>
                        <TouchableOpacity style={styles.edit} onPress={() => console.log('Edit')}>
                        <EditIcon />
                        </TouchableOpacity>
                        <TouchableOpacity style={styles.delete} onPress={() => console.log('Delete')}>
                        <DeleteIcon />
                        </TouchableOpacity>
                    </View>
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        <Text style={{ flex: 0.3, textAlign: 'center', fontFamily: 'Montserrat-Regular', color: colors.light_gray }}>
                        {formatDate(new Date(bet.data))}
                        </Text>
                        <View style={{ flex: 0.7, backgroundColor: colors.white, height: 65, borderRadius: 10, flexDirection: 'row' }}>
                        <View style={styles.icon}>
                            <View style={{ width: 40, height: 40, borderWidth: 3, borderColor: colors.light_gray, borderRadius: 50, justifyContent: 'center', alignItems: 'center' }}>
                                {
                                    bet.sport === Sport.FOOTBALL ? <FootballIcon /> : bet.sport === Sport.BASKET ? <BasketIcon/> : bet.sport === Sport.TENNIS ? <TennisIcon/> : <OtherIcon/>
                                }
                            </View>
                        </View>
                        <View style={styles.data}>
                            ...
                            <Text style={{ fontFamily: 'Montserrat-Regular', fontSize: 12, color: colors.light_gray }}>
                            Stake: <Text style={{ fontFamily: 'Montserrat-SemiBold' }}> {bet.stake}%</Text>
                            </Text>
                            <Text style={{ fontFamily: 'Montserrat-Regular', fontSize: 12, color: colors.light_gray }}>
                            Amount: <Text style={{ fontFamily: 'Montserrat-SemiBold' }}> €{bet.amount.toFixed(2).replace('.', ',')}</Text>
                            </Text>
                        </View>
